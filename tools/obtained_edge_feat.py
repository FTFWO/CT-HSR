...
parser.add_argument("--imdb_path", type=str, default="data/imdb/m4c_textvqa/imdb_train_ocr_en.npy", )
...
for fea in tqdm(imdb[1:]):
    ...
    for m, obj_box_m in enumerate(obj_bbox):
        rcx, rcy, rw, rh = (obj_box_m[0] + obj_box_m[2] + 1) / 2, (obj_box_m[1] + obj_box_m[3] + 1) / 2, (
                    obj_box_m[2] - obj_box_m[0] + 1), (obj_box_m[3] - obj_box_m[1] + 1)
    ...
    for a, ocr_box_a in enumerate(ocr_bbox):
        rcx, rcy, rw, rh = (ocr_box_a[0] + ocr_box_a[2] + 1) / 2, (ocr_box_a[1] + ocr_box_a[3] + 1) / 2, (
                    ocr_box_a[2] - ocr_box_a[0] + 1), (ocr_box_a[3] - ocr_box_a[1] + 1)
        if a >= 50:
            break
